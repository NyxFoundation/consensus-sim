...
by the GitHub Actions workflow at `.github/workflows/deploy.yml`. That
workflow is a thin shim — check the repository out, install Node, run one
script, publish `dist/` — and the script is the repository's own:
...
this repository keeps is therefore that `scripts/ci.sh` exits 0 from a clean
...
## Development workflow (IDD)

This repository is developed with
[Ideal-Driven Development (IDD)](https://github.com/banr1/ideal-driven-development).
`IDEAL.md` is the human-owned statement of the state to reach (conditions
C1–C36, each with its verification); `.idd/` holds the condition ledger,
the shared knowledge, the per-cycle records and the journal. The `idd`
command runs from this directory:

```bash
idd doctor                # agent CLI, guardrail wiring, IDEAL.md structure, clean tree
idd once                  # one cycle; `idd loop` runs up to 25
idd status                # state, stop reason, next step (`idd show N` prints a record)
idd refine                # rewrite IDEAL.md through an interview (also where cycle questions are answered)
idd resume --note "…"     # record a human intervention and clear a stop state
```

No `CLAUDE.md` or `.claude/` lives here: the cycle prompt injects the
context and the guardrails are wired at launch. `essences/deep-research-report.md`
is human-owned source material referenced by `IDEAL.md` and the attack
library.

Until September 2026 the project was driven by Atlas Builder from a sibling
control plane, with the project in a `consensus-sim/` subdirectory. Its
essence is readable as `git show 41169f7:consensus-sim/ESSENCE.md`, and
`IDEAL.md` keeps its numbering (C1–C32 are the old 必須対応事項 1–32), so the
old numbers cited in code comments still resolve.
