...
describe('presets and individual parameters (成功条件 14)', () => {
  it('starts on merge and switches every value with a preset', async () => {
    ...
    expect(pressed('justified 切替窓')).toBe('on')
    expect(pressed('unrealized justification')).toBe('off')
    ...
    expect(input('proposer boost').value).toBe('0.4')
    expect(pressed('justified 切替窓')).toBe('off')
    expect(pressed('unrealized justification')).toBe('on')
  })
  ...
  it('leaves the preset once a single value departs from it, and returns on reselect', async () => {
    ...
    // The two switches are independent: both can be on at once.
    await press('unrealized justification', 'on')
    expect(pressed('justified 切替窓')).toBe('on')
    expect(pressed('unrealized justification')).toBe('on')
    await press('justified 切替窓', 'off')
    expect(pressed('justified 切替窓')).toBe('off')
  ...
  it('hides the leak schedule while the leak is off and restores the default when it returns', async () => {
    await press('inactivity leak', 'off')
    expect(container.querySelector('input[aria-label="inactivity leak N"]')).toBeNull()
    expect(pressed('プロトコルプリセット')).toBe('')
    await press('inactivity leak', 'on')
    expect(input('inactivity leak N').value).toBe('4')
    expect(input('inactivity leak r').value).toBe('0.25')
    expect(pressed('プロトコルプリセット')).toBe('merge')
  })
