...
- ただし周回の実行環境は次を拒否する(周回 1・2 で再現): `bash <script>` の起動、プロジェクト外への `cd`、プロジェクト外の `rm`、リダイレクトや `;` 連鎖を含む複合コマンド、`node -p`。複製の作成(mktemp / cp)は通るが clean copy の `scripts/ci.sh` は起動できない。拒否されたらコマンドを単純化して 1 つずつ試し、それでも駄目なら条件を blocked にして記録に残す(拒否の回避を試みない)。
- `scripts/ci.sh` の手順 2〜4 は、作業木で単独コマンドとして同じ検査ができる: `npm test` / `npm run build` / `grep -Ec '(src|href)="(/|[a-zA-Z][a-zA-Z0-9+.-]*:)' dist/index.html` が 0 / `grep -Ec '(src|href)="\./assets/' dist/index.html` が 2。`grep -q` は出力が無く終了コードも見えないので `-c` か `-o` を使う。再現できないのは手順 1(node_modules を持たない複製での `npm ci`)だけ。
